...
module.exports.itemsCart = function (userID,cartId) {
    return new Promise(function (resolve, reject) {
        let sql = "SELECT `item_cart`.`id` as 'itemId',`item_cart`.*, `products`.* FROM `item_cart` INNER JOIN `products` on `products`.`id`=`item_cart`.`product_id` WHERE `item_cart`.`user_id`=? and `item_cart`.`cart_id`=?";
