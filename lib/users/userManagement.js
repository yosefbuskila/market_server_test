...
module.exports.login = function (email, password) {
    return new Promise((resolve, reject) => {
        let ranstr;
        query("SELECT `id`,`password` FROM `users` WHERE `email`= ?  ", [email]).then(function (result) {
            if (result.length ==!1) {
...
// module.exports.login('ayoasefssd30es53@gmail', '1234').then((token) => console.log('OK',token)).catch(() => console.log('notOK'))


module.exports.chakConnect = function (id, token) {
    return new Promise((resolve, reject) => {
        query("SELECT `time` FROM `logintoken` WHERE `id`= ? and `token` = ? ", [id,token]).then(function (result) {
            if (result.length ==!1) {
                reject()
                return Promise.reject();
            }
                else return result[0].time;   
    }).then(time=>{
        console.log(time,'t')
        resolve()
    })
...
module.exports.chakConnect(80, 'x9S5iYnOuFXw3s26wprTsFIK6WliUOde').then((res) => console.log('OK'),function (res)  {console.log('notOK')})
